...
def get_abs_quantities(stats_by_time,label_list,index_low=0, index_high=None, step=1):
    ...
    for label in label_list:
        if label not in [('homo',0),('homo',1),('hetero',0),('hetero',1)]:
...
def get_data(stats_by_time,data_type,index_low=0, index_high=None, step=1):
    ...
    'population_size' or 'mutation_counter'.
    
    """
    valid_data_types = ['time','trait','population_size','mutation_counter']
...
def get_number_of_active_families(stats_by_time,index_low=0, index_high=None, step=1):
    """Returns a list with the number of active families in the time interval.
    ...
    for count in stats_by_time[index_low:index_high:step]:
            data.append(len(count['active_families']))
